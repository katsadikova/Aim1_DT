...
d<-data.frame(read_sas("/Users/Kat/Library/CloudStorage/OneDrive-HarvardUniversity/VDI/DT/data/dat_hotdeck_int_ext_extra.sas7bdat"))
...
vars<-names(d[,c(13,14,18:20,28:46,51:53)])
...
hist(db$TANNER_STAGE)
hist(dg$TANNER_STAGE)
...
cog_dep<-bind_rows(cog_dep_hima, .id = "outcome") %>%
  mutate(exposure = "Cognitive Deprivation")
emo_dep<-bind_rows(emo_dep_hima, .id = "outcome") %>%
  mutate(exposure = "Emotional Deprivation")
phys_dep<-bind_rows(phys_dep_hima, .id = "outcome") %>%
  mutate(exposure = "Physical Deprivation")

results_adj <- rbind(thr,dep,cog_dep,emo_dep,phys_dep) 
