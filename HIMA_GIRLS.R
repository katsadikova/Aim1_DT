...
for (i in names(db)[c(48,49)]){
  ...
  Y = dg[[i]]
  print(summary(Y))
  M = dg %>% dplyr::select(attentothreat,
  ...
  COV.XM = dg %>% dplyr::select(S1AGE, POV_CHRONICITY, cesd_mom_max)
  COV.MY = dg %>% dplyr::select(S1AGE, POV_CHRONICITY, FIL_DEPRIVATION, cesd_mom_max)
...
dep_hima <- list()
for (i in names(dg)[c(48,49)]){
  X = dg$FIL_DEPRIVATION
  #Standardize outcome to have mean 0, std 1
  Y = dg[[i]]
  M = dg %>% dplyr::select(attentothreat,
                          ADAPTATION,STROOP_FEAR, STROOP_HAPPY, 
                          ...
                          STROOP_ACC, rs_rt, TotalStars)
  COV.XM = dg %>% dplyr::select(S1AGE, POV_CHRONICITY, cesd_mom_max)
  COV.MY = dg %>% dplyr::select(S1AGE, POV_CHRONICITY, FIL_THREAT, cesd_mom_max)
...
write.csv(results_adj, file=here("/Users/Kat/Dropbox/A_DISSERTATION/Aims/Aim1/Aim1_DT/results/HIMA_GIRLS_results_adj_INT_EXT_TESTING_NO_NOGO.csv"))
